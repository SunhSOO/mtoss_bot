...
async def cleanup_trace(engine: AsyncEngine, signal_id: UUID, account_id: UUID) -> None:
    async with AsyncSession(engine) as session:
        await session.execute(
            delete(AuditEventRecord).where(AuditEventRecord.trace_id == signal_id)
        )
        await session.execute(
        ...
        await session.execute(
            delete(OrderIntentRecord).where(OrderIntentRecord.signal_id == signal_id)
        )
...
async def test_repeated_post_persists_one_correlated_queued_slice_and_returns_409() -> None:
    ...
    try:
        ...
        try:
            await cleanup_trace(engine, signal_id, account_id)
        finally:
